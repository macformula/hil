...
def serve(rpConfig: dict):
    ...
    # Insta
    repo_handler = RepoHandler(pages_repo_dir=rpConfig[GITHUB_PAGES_REPO_KEY],
    ...
    ra = ResultAccumulator(tags_fp=rpConfig[TAGS_FP_KEY],
...
def main():
    ...
    if not config_fp:
        print('Error: Configuration file path is required. Use --config="/path/to/config".')
        sys.exit(1)
